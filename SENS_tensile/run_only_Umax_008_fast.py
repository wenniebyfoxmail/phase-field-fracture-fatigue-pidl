#!/usr/bin/env python3
"""
run_only_Umax_008_fast.py — 单独跑 coeff=3.0, Umax=0.08（速度优化版）
用途：sweep 中 Umax=0.08 还没排到时，先 kill 主进程，用此脚本独立跑 0.08
数值参数与 run_sequential_coeff3.py 完全一致；I/O 和 NN 编译走快版：
  - log_every_n_cycles=5
  - tqdm 全局禁用
  - network_dict["compile"]=True
  - optim_rel_tol 不变（保 coeff=1.0 baseline 可比）

输出目录：与原 sweep 一致，所以可以与其它 case 一起后处理。

用法（在 SENS_tensile/ 目录下）:
  PYTHONIOENCODING=utf-8 nohup /c/Users/xw436/Python310/python.exe -u run_only_Umax_008_fast.py \\
       > run_only_Umax_008_fast.log 2>&1 &
"""
...
network_dict = {
    ...
    "compile"     : False,            # ★ 关：Windows 无 triton，反复 fallback 反而更慢
}
optimizer_dict = {
    "weight_decay"         : 1e-5,
...
# ── Case 定义 ─────────────────────────────────────────────────────────────────
CASES = [
    {"disp_max": 0.08, "n_cycles": 600},   # 仅此 case
]
