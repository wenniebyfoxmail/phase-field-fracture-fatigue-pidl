...
def _resolve_mesh_file(here: Path, filename: str) -> str:
    ...
    # Worktrees and copied experiment dirs may not carry ignored .msh files.
    # Reuse the nearest parent-level SENS_tensile mesh copy when available.
...
def main():
    ...
    p.add_argument("--tip-local", action="store_true",
                   help="Enable the compact moving tip-local correction head together with S2.")
    p.add_argument("--window-radius", type=float, default=0.05,
                   help="Tip-local taper radius when --tip-local is enabled.")
    ...
    p.add_argument("--static-window", action="store_true",
                   help="Keep the local correction window fixed at --tip-x0/--tip-y0.")
    p.add_argument("--follow-y-mode", choices=("centerline", "tip"), default="centerline",
                   help="Y coordinate policy for moving tip-local window.")
    ...
    config.tip_local_net_dict["enable"] = bool(args.tip_local)
    ...
    config.tip_local_net_dict["x_tip"] = float(args.tip_x0)
    config.tip_local_net_dict["y_tip"] = float(args.tip_y0)
    ...
    if args.tip_local:
        print(
            f"  tip-local= {args.tip_hidden_layers}x{args.tip_neurons} | "
            f"arch={args.tip_arch} | window={args.window_radius} | "
            f"mode={args.output_mode} | follow={config.tip_local_net_dict['follow_tip']}"
        )
