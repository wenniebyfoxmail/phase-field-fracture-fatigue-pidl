#!/usr/bin/env python3
"""
run_sequential.py — 顺序跑多个 cyclic fatigue case，无需手动干预
满足断裂条件（crack_length >= 0.46 AND E_el < 20%×E0）自动停止

用法（在 upload code/SENS_tensile/ 目录下）:
  nohup python3 run_sequential.py > run_sequential.log 2>&1 &

当前配置:
  Case G: Umax=0.09, n_cycles=500
  Case H: Umax=0.11, n_cycles=200
"""
...
optimizer_dict = {"weight_decay": 1e-5, "n_epochs_RPROP": 10000, "n_epochs_LBFGS": 0,
                  "optim_rel_tol_pretrain": 1e-6, "optim_rel_tol": 5e-7}
...
# ── Case 定义 ─────────────────────────────────────────────────────────────────
CASES = [
    {"disp_max": 0.09, "n_cycles": 500},   # Case G
    {"disp_max": 0.11, "n_cycles": 200},   # Case H
]
...
for case_cfg in CASES:
    ...
    print(f"\n{'='*60}")
    print(f"Starting: Umax={disp_max}, n_cycles={n_cycles}")
