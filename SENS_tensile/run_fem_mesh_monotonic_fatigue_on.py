#!/usr/bin/env python3
"""Run strict FEM-mesh PIDL with fatigue on and monotonic U=0.2 loading.

This is the Case C consistency pair for
``run_fem_mesh_monotonic_fatigue_off.py``.  With
``loading_type='monotonic'``, the fatigue-history update intentionally leaves
``alpha_bar`` unchanged, so this run should match the fatigue-off monotonic
trajectory while keeping the fatigue-on code path active.
"""
...
def main() -> None:
    ...
    p.add_argument("--tag", default="softHist0_monoU02_fatigueOn")
    ...
    config.fatigue_dict["fatigue_on"] = True
    ...
    dir_name = (
        ...
        + "_fatigue_on"
        + f"_{fat.get('accum_type', 'carrara')}"
        + f"_{fat.get('degrad_type', 'asymptotic')[:3]}"
        + f"_aT{fat.get('alpha_T', 0.094)}"
        + f"_N{fat.get('n_cycles', len(config.disp))}"
        + f"_R{fat.get('R_ratio', 0.0)}"
        + f"_Umax{fat.get('disp_max', 0.2)}"
        + "_mono"
        + f"_femmesh_{tag}"
    ...
    with open(config.model_path / "model_settings.txt", "w", encoding="utf-8") as f:
        f.write("runner: run_fem_mesh_monotonic_fatigue_on.py\n")
        f.write("fatigue_on: True\n")
    ...
    print("=" * 72)
    print("Strict PIDL FEM-mesh monotonic fatigue-on runner")
