#!/usr/bin/env python3
"""
run_sequential_coeff3_fast.py — 串行跑 coeff=3.0 sweep（速度优化版）
与 run_sequential_coeff3.py 数值参数完全一致，只动 I/O 和 NN 编译：
  - log_every_n_cycles=5 → 普通日志降频 5×（关键事件 Fracture?/Dense sampling/Confirmed 仍每步）
  - tqdm 全局禁用（在 source/fit.py 已修改）
  - network_dict["compile"]=True → torch.compile(reduce-overhead) 减少 launch overhead
  - optim_rel_tol 不变（保与 coeff=1.0 baseline 对比一致性）

用法（在 SENS_tensile/ 目录下）:
  PYTHONIOENCODING=utf-8 nohup /c/Users/xw436/Python310/python.exe -u run_sequential_coeff3_fast.py \\
       > run_sequential_coeff3_fast.log 2>&1 &
"""
...
network_dict = {
    ...
    "compile"     : True,             # ★ 速度：torch.compile（首次 ~30s 编译）
}
optimizer_dict = {
    "weight_decay"         : 1e-5,
...
def make_fatigue_dict(disp_max, n_cycles):
    return {
        ...
        "log_every_n_cycles"     : 5,   # ★ 速度：每 5 步打印一次普通日志（关键事件不受控）
    }
