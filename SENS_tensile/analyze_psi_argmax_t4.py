#!/usr/bin/env python3
"""
analyze_psi_argmax_t4.py — generic ψ⁺ argmax stationarity diagnostic.

Originally written for α-2 (multi-head smooth-gate); also works for α-3
(XFEM-jump Heaviside) or any method that saves psi_argmax_vs_cycle.npy.
...
Targets per design_alpha2_multihead_apr28.md / design_alpha3_xfem_jump_apr29.md:
  baseline    ~5-10% modal
  α-1 mesh    ~10-20% modal
  α-2 default 0.30 / tighter-gate 0.30 (both FAIL)
  α-3 10-cycle smoke 0.50 (transient ramp-up; needs N=300 to read steady-state)
  α-3 PASS    ≥ 0.70 over full life (matching FEM 0.82 modal)

Usage:
    python analyze_psi_argmax_t4.py <archive_dir>
    python analyze_psi_argmax_t4.py <archive_dir1> <archive_dir2> ...   # compare

Apr 30 note: ported from claude/exp/alpha2-multihead branch to main +
renamed (was analyze_alpha2_t4.py) so watchers (chained_v8/v9+) can find
it on any branch without checking out a specific architecture branch.
"""
