#!/usr/bin/env python3
"""run_fem_anchor_bc_umax.py — GRIPHFiTH essential-BC alignment runner.

Controlled alignment Variant 5:
  keep the Phase 1 PIDL baseline unchanged except for the displacement ansatz.

Baseline PIDL fixes u_x on the top and bottom edges because the displacement
correction is multiplied by a top-bottom bubble and vertical loading has
cos(theta)=0. GRIPHFiTH Phase 1 instead fixes u_x only at the bottom-left node,
while bottom/top prescribe only u_y. This runner uses an ansatz that mirrors
that essential BC:

  - v=0 on bottom,
  - v=Umax on top,
  - u=0 only at (x_min, y_min),
  - u is free on the rest of top/bottom.
"""
...
def main() -> int:
    ...
    p.add_argument("--n-cycles", type=int, default=120)
    ...
    with open(config.model_path / Path("model_settings.txt"), "w") as f:
        ...
        f.write(f"\nalpha_T: {fat['alpha_T']}")
        f.write(f"\nR_ratio: {fat['R_ratio']}")
        f.write("\n[runner] run_fem_anchor_bc_umax.py")

    print("=" * 72)
    print("Controlled alignment Variant 5: FEM-anchor BC runner")
