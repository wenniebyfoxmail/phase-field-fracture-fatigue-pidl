#!/usr/bin/env python3
"""Run strict FEM-mesh PIDL with a controlled fatigue-history driver.

This discriminator keeps the variational solve, mesh, initial crack convention,
and fracture criterion aligned with the strict FEM-mesh soft-hist0 benchmark.
Only the post-solve fatigue-history update driver changes:

* current_active: historical PIDL default, g(alpha_current) * psi_raw_current.
* lagged_g: uses g(hist_alpha_previous) * psi_raw_current.
* raw: uses psi_raw_current directly as a diagnostic ceiling.

The `lagged_g` branch tests whether the coupled NN solve relaxes alpha in a way
that kills the active driver before the fatigue accumulator can see the work.
The `raw` branch is not a proposed physics law; it is a ceiling test.
"""
...
def main() -> None:
    ...
    p.add_argument("--tag", default="softHist0_histDriver",
                   help="Archive tag suffix; mode is appended automatically")
    p.add_argument("--history-driver-mode", choices=("current_active", "lagged_g", "raw"),
                   default="lagged_g")
    p.add_argument("--fracture-confirm-cycles", type=int, default=3)
    p.add_argument("--plot-every", type=int, default=20)
    p.add_argument("--diag-cycles", default="0,1,2,19,39,68",
    ...
    # Strict benchmark controls: same as run_fem_mesh_umax.py.
    ...
    config.fatigue_dict["element_diagnostics"] = {
        ...
        "dense_sampling": True,
        "on_fracture": True,
        "dir": f"element_diagnostics_history_driver_{args.history_driver_mode}",
    }
    ...
    suffix = (
        f"_femmesh_{tag}_{args.history_driver_mode}"
    ...
    with open(config.model_path / "model_settings.txt", "w", encoding="utf-8") as f:
        ...
        f.write("purpose: controlled post-solve fatigue-history driver discriminator\n")

    print("=" * 72)
    print("PIDL FEM-mesh history-driver discriminator")
    print(f"  U_max       = {args.umax} | n_cycles = {args.n_cycles} | seed = {args.seed}")
    print(f"  FEM mesh    = {fem_mesh}")
    print(f"  coarse mesh = {coarse_mesh}")
    print(f"  mode        = {args.history_driver_mode}")
