...
- **LTPP spatial-registration track（二维路线暂时关闭）**：外部审核后一次性运行的 `06-1253` frozen-grid audit 只有 `2/8` 日期通过；1995、2001、2007 控制线不足，1991、1997、2003 又未通过预设空间误差门。因此当前 deskew/crop/resize carrier 不得进入二维位置或裂缝尖端算法；这不追溯否定独立的标量长度任务。完整证据维护在 `docs/experiments/ltpp_geoforecast_spatial_registration_track.md`。
