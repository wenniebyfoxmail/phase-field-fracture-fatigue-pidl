...
3. **FEM-surrogate transition operator** (DIAGNOSTIC 7/18) — A frozen Taobo data-efficiency matrix on the only complete c1--c89 trajectory tested `C_train={20,40,60,67}` and `k={1,3,5,10}` with persistence, pointwise and parameter-matched temporal mesh models. c20 suffices for tested one-step reliability, but persistence is already strong. k3 is only a nominal global tie with k1; longer context can extend the full c76-origin gate to h3, then support localization fails at h4/c80. Every k catastrophically misses c89 (about 97x FEM support area), so more history of the same four hidden fields is not the missing transition signal. Keep the result single-trajectory/assimilation-ineligible; do not promote before physical multi-trajectory data. Artifact: `docs/fem_cycle_forecast_data_efficiency_2026-07-18.md`.
