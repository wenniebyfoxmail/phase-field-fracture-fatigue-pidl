...
- Request 28 的 c121/c82/c58 s5 -> c122/c83/c59 s4 三个 replay pair 已通过独立状态语义审计，但冻结的 plain-gradient vs crack-enriched 两坐标 gate 为 `ENRICHED_RETROSPECTIVE_ENCODING_NEGATIVE`：ENRICHED 仅赢 PLAIN `3/6`、赢 Umax-only `3/6`，并有结构性非单调校准。关闭 pairing/ROI/Williams/KAN rescue 与 event-relative earlier-origin 请求；若未来重开，只能使用固定 calendar origins、独立 trajectories 和单独 preregistered fatigue-exposure coordinate。完整证据见 `docs/experiments/request28_xdem_observability_track.md`。
