...
2. **Hard-recovery c89 field mechanism** (ACTIVE diagnostic 7/18) — The sealed c87 observation ladder finds conditional c89 closure from 10% adaptive raw-energy probes: log-MAE `0.0727`, correlation `0.9266`, FEM-p99 IoU `0.9356`, area ratio `0.9402`; the same tier survives the fixed 10% noise stress. Reaction + crack mask fails (`corr=0.8714`) and 5% probes fail (`0.8968`), so geometry/global load do not identify near-core amplitude and density/location matter. This is a one-trajectory synthetic state-estimation result only: `psi_raw` must be strain-reconstructed with a declared split, other hidden channels stay prior-conditioned, and sparse uncertainty is under-covered (`90% coverage=0.094`). Next gate is physical-trajectory holdout with calibrated uncertainty, not a learned/material inverse. Artifacts: `docs/inverse_fracture_state_assimilation_2026-07-18.md`; local `analysis/inverse_fracture_state_assimilation_20260718/decision.md`.
