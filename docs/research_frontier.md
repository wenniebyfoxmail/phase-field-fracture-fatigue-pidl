...
**LTPP load-only incremental track（候选、未冻结）**：拟只比较 `G = 几何+预测间隔` 与 `G+L = G+单一年度 ESAL`，不加入气候、水分、结构、FWD 或其他交通变量；当前仅完成协议草案，等待 Pro 审核，禁止拟合。完整协议维护在 `experiments/ltpp_geoforecast_load_only_incremental_track_20260806.md`。
