...
1. **Representation-localization discriminator** (ACTIVE 5/30) — Sampling/refinement/loss/irreversibility did not close the field gap. Tip-local MLP/Fourier/SIREN Exp18 and strict head-staging change `N_f`/scalar metrics but leave the local `alpha_bar`/active-`psi+` feedback broken. Current clean test is a strict FEM-mesh compact-support local patch: global MLP + `chi(r)*patch_MLP`, with patch-only warm-up then joint solve. Artifact: `docs/local_patch_discriminator_2026-05-30.md`.
