...
- **LTPP spatial-registration track（二维路线暂时关闭）**：外部审核后一次性运行的 `06-1253` frozen-grid audit 只有 `2/8` 日期通过；1995、2001、2007 控制线不足，1991、1997、2003 又未通过预设空间误差门。2026-08-10 的 no-OCR complete-grid MVP 与独立 outer-boundary/topology MVP v2 都在 8 个已见 development 页面全部 fail-closed，未产生可用候选框；不得调参重跑或进入二维路线。因此当前 deskew/crop/resize carrier 不得进入二维位置或裂缝尖端算法；这不追溯否定独立的标量长度任务。完整证据维护在 `docs/experiments/ltpp_geoforecast_spatial_registration_track.md`。
