...
1. **Representation-localization discriminator** (ACTIVE 5/30) — Sampling/refinement/loss/irreversibility did not close the field gap. Tip-local MLP/Fourier/SIREN Exp18, strict head-staging, additive one-patch local correction, and FBPINN-chain change `N_f`/propagation mode but leave the active-driver/history mechanism broken. First offline mesh-GNN residual screen gives a graph-only signal for held-out c69 `psi_active` (log-MAE 0.498→0.364 vs MLP 0.466) but direct `alpha_bar` correction is negative; treat as a diagnostic sidecar idea, not closure. Artifacts: `docs/local_patch_discriminator_2026-05-30.md`, `docs/fbpinn_chain_discriminator_2026-05-30.md`, `docs/mesh_gnn_residual_discriminator_2026-06-16.md`.
2. **M2S framework validation** (ACTIVE 5/31) — First single-trajectory soft-hist0 rung is complete, and the next-stage feasibility package now adds leave-one-trajectory-out holdouts plus sparse/noisy observation tests. Legacy five-Umax scalar holdout is the useful stress test: damage-only RUL MAE is ~82-86 cycles, while state-driver proxies improve to ~35 cycles; strict soft-hist0 cadence field holdout is still too easy because trajectories are nearly identical. Artifacts: `docs/m2s_framework_validation_2026-05-31.md`, `docs/m2s_next_stage_feasibility_2026-05-31.md`, `_analysis_m2s_next_stage_20260531/`.
...
For forward PIDL, do not promote another representation tweak unless it improves the active-driver/process-zone gate in `docs/active_driver_process_zone_metric_2026-06-28.md`, not just scalar `N_f`. The mesh-GNN sidecar is only worth a second pass if targeted to `psi_active` with stricter holdouts; do not use it as direct `alpha_bar` correction. Metric plan/results started in `docs/field_level_comparison_metric_plan.md`, `docs/field_level_metric_results_2026-05-28.md`, `docs/aligned_fem_pidl_reference_tiers_2026-05-30.md`, `docs/staged_alpha_discriminator_2026-05-30.md`, `docs/local_patch_discriminator_2026-05-30.md`, `docs/fbpinn_chain_discriminator_2026-05-30.md`, `docs/mesh_gnn_residual_discriminator_2026-06-16.md`, and `docs/inverse_problem_experiments_2026-05-31.md`; element/nodal definitions are pinned in `docs/fem_pidl_element_nodal_definitions_2026-05-28.md`. For framework validation, the next meaningful production step is to export strict soft-hist0/reverseBC full-field trajectories across `Umax`, `alpha_T`, and precrack severity, then rerun `_analysis_m2s_next_stage_20260531` on that stricter family.
