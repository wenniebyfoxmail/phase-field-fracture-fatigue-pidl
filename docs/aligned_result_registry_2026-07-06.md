...
| c87 inverse fracture-state observation ladder | `diagnostic` | Determines the predeclared sensing burden needed to repair the frozen c87->c89 energetic forecast on one eta0 trajectory. | Conditional state-estimation evidence only: 10% adaptive synthetic raw-energy probes pass sealed c89 gates, while reaction+mask and 5% probes fail; uncertainty remains under-covered and no material inverse is identified. | `docs/inverse_fracture_state_assimilation_2026-07-18.md`; local `analysis/inverse_fracture_state_assimilation_20260718/decision.md`. |
