...
| FEM cycle-forecast data-efficiency matrix | `diagnostic` | Measures within-trajectory next-cycle and recurrent rollout sufficiency over `C_train={20,40,60,67}` and `k={1,3,5,10}`. c20 suffices for tested one-step reliability; longer history extends the complete near-event gate to h3 but every k misses c89 with about 97x support area. | Use to bound single-trajectory temporal forecasting and motivate an additional regime-sensitive observation/state. Do not claim physical-trajectory or geometry generalization. | `docs/fem_cycle_forecast_data_efficiency_2026-07-18.md`; local `analysis/fem_cycle_forecast_data_efficiency_20260718/analysis/decision.md`. |
