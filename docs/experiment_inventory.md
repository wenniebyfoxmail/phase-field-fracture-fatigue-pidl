...
| **2026-06 c1 alternate-minimisation micro-solve** | Mac+Taobo | New strict FEM-mesh c1 probe alternates `freeze alpha -> solve u/v` and `freeze u/v -> solve alpha` for three rounds from the residual-stiffness post-pretraining checkpoint. It exports every substage with epsilon/psi/alpha fields plus `E_el/E_d/E_hist` and `grad_logEel/grad_logEd/grad_logEhist`. | 🏃 launched/awaiting Taobo result; tests whether the first-cycle raw-field overshoot is born in uv equilibrium, alpha/degradation feedback, or joint optimiser timing |
