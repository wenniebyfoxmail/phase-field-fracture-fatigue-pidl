...
| **2026-06 frozen-alpha elastic c1 discriminator** | Mac+Taobo | Fixed analytic soft-hist0 alpha, displacement-only elastic optimisation from residual-stiffness post-pretraining checkpoint. `E_el` reduced 2.23e-2→7.19e-3. Against FEM22 c1 peak: coupled c1 was too hot (`psi_raw` tip2=2.34×, max=4.07×), while frozen-alpha became too cold (`psi_raw` tip2=0.25×, max=0.0527×; `eps_eq` tip2=0.581×). | ✅ diagnostic complete: initial alpha/precrack alone does not force the hotspot; first coupled elastic-damage optimisation path creates the raw-field overshoot. Next: c1 alternate-minimisation micro-solve |
| **2026-06 c1 alternate-minimisation micro-solve** | Mac+Taobo | Strict FEM-mesh c1 probe alternated `freeze alpha -> solve u/v` and `freeze u/v -> solve alpha` for three rounds from the residual-stiffness post-pretraining checkpoint. `r1_uv` cooled the raw hotspot (`psi_raw_max` 2704→53.9; 0.079× FEM max), `r1_alpha` did not restore it, then uv re-equilibration around updated alpha rebuilt it (`r2_uv` 571.6, `r3_uv` 2409.9; `eps_eq` tip2≈1.01× FEM and `psi_active` tip2≈1.04× FEM at `r3_uv`, but raw max still 3.51× FEM). Alpha-stage gradients were dominated by irreversibility (`grad_logEhist≈9e2-1.1e3`). | ✅ diagnostic complete: first-cycle hotspot is a coupled alpha→uv feedback/timing issue, not pure displacement-only equilibrium or alpha-only motion |
| **2026-06 c1 alt-min branch matrix** | Mac+Taobo | Four follow-up schedules tested priority-1/2 ideas: `shortAlpha_a200`, `shortAlpha_a500`, `smallStagger_r6`, and `alphaLR1e6_r6`. All final uv states kept tip `eps_eq` and `psi_active` close to FEM (≈1.01-1.07×) but left `psi_raw_max` high (3.53-3.85× FEM). Lowering alpha RPROP LR to `1e-6` barely changed the result; final alpha-stage `grad_logEhist` stayed ≈1.0e3. | ❌ simple alpha softening / smaller stagger / lower alpha LR do not fix the c1 feedback loop; next lever should target irreversibility/update formulation or local stiffness/degradation response |
