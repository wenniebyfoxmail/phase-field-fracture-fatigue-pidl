...
| **2026-06 frozen-alpha elastic c1 discriminator** | Mac+Taobo | New runner freezes alpha to the analytic soft-hist0 initial precrack and optimises only the displacement field through elastic energy, starting from the residual-stiffness branch's post-pretraining checkpoint. Outputs epsilon/psi fields for FEM22 c1 peak comparison. | 🏃 launched/awaiting Taobo result; tests whether the first `psi_raw` hotspot is born in the elastic displacement solve before alpha/fatigue feedback |
