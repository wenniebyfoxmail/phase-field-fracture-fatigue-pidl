...
function testFinalizerBindsQualificationAndRejectsRuntimeDrift(testCase)
[outputRoot, launchReceipt, dependencies, cleanup] = validPackage(); %#ok<ASGLU>
summary = finalizeFixture(outputRoot, "T2_material_state", ...
    launchReceipt, dependencies);
verifyEqual(testCase, summary.qualification_receipt_sha256, repmat('4', 1, 64));
verifyEqual(testCase, summary.runtime_initial_sha256, approvedInitialHash());

[outputRoot, launchReceipt, dependencies, cleanup] = validPackage(); %#ok<ASGLU>
runtimePath = fullfile(outputRoot, 'FAMILY_RUNTIME_RECEIPT.json');
runtime = jsondecode(fileread(runtimePath));
runtime.qualification_receipt_sha256 = repmat('9', 1, 64);
delete(runtimePath);
write_toy_road_json(runtimePath, runtime);
verifyError(testCase, @() finalizeFixture(outputRoot, ...
    "T2_material_state", launchReceipt, dependencies), ...
    'toyRoad:PackageValidationFailed');
end
...
qualification = struct( ...
    ...
    'case_id', cfg.case_id, ...
    'qualification_receipt_sha256', repmat('4', 1, 64), ...
    'runtime_lock_sha256', repmat('a', 1, 64), ...
    'runtime_initial_sha256', approvedInitialHash(), ...
    'source_commit', '355d4c83fefc2db88c32031a2dd2623b3de85c89', ...
    'q1_receipt_sha256', repmat('b', 1, 64), ...
    'q2_receipt_sha256', repmat('d', 1, 64), ...
    'parent_lock_sha256', parentLockHash, ...
    'parent_cycle1_sha256', repmat('f', 1, 64), ...
    'mesh_ordering_sha256', repmat('1', 1, 64));
write_toy_road_json(fullfile(outputRoot, 'FAMILY_RUNTIME_RECEIPT.json'), ...
    qualification);
...
receipt = struct( ...
    ...
    'runtime_lock_sha256', qualification.runtime_lock_sha256, ...
    'runtime_initial_sha256', qualification.runtime_initial_sha256, ...
    'q1_receipt_sha256', qualification.q1_receipt_sha256, ...
    'q2_receipt_sha256', qualification.q2_receipt_sha256, ...
    'qualification_parent_cycle1_sha256', ...
        qualification.parent_cycle1_sha256, ...
    'qualification_mesh_ordering_sha256', qualification.mesh_ordering_sha256, ...
