function receipt = validate_qualification_receipts(qualificationRoot, ...
        ...
        parentLockPath, parentRoot, outputPath, testContext)
...
testOnly = nargin == 9;
if testOnly
    requireTestOnlyCaller();
    requireExactFields(testContext, {'evidence_lock_path', ...
        'evidence_lock_sha256','parent_receipt','q1_recomputed_metrics'});
    evidenceLockPath = char(testContext.evidence_lock_path);
    expectedEvidenceLockSha256 = char(testContext.evidence_lock_sha256);
    if ~isempty(outputPath)
        error('rebuiltMexQualification:TestOnlyCannotPublish', ...
            'Test-only qualification cannot publish a family receipt.');
    end
else
    evidenceLockPath = fullfile(handoff, 'QUALIFICATION_EVIDENCE_LOCK.json');
    expectedEvidenceLockSha256 = repmat('0', 1, 64);
    if strcmp(expectedEvidenceLockSha256, repmat('0', 1, 64))
        error('rebuiltMexQualification:EvidenceLockUnsealed', ...
            ['QUALIFICATION_EVIDENCE_LOCK is not sealed. Real Task6 Q1/Q2 ', ...
            'qualification and a source reseal are required.']);
    end
    if ~isfile(evidenceLockPath) || ...
            ~strcmp(fileSha256(evidenceLockPath), expectedEvidenceLockSha256)
        error('rebuiltMexQualification:EvidenceLockMismatch', ...
            'The canonical qualification evidence lock is absent or changed.');
    end
end
evidence = validateEvidenceLock(evidenceLockPath, ...
    expectedEvidenceLockSha256, qualificationRoot);
...
if ~isfolder(parentRoot)
    error('rebuiltMexQualification:IdentityMismatch', ...
        'The exact parent root is absent.');
end
...
if testOnly
    recomputedQ1 = testContext.q1_recomputed_metrics;
    recomputedInputSha = textField(evidence.identities, 'q1_input_sha256', 64);
else
    [recomputedQ1, recomputedInputSha] = ...
        recompute_q1_initial_qualification_metrics(runtimeLockPath, ...
        runtimeRoot, sourceRoot);
end
if ~semanticEqual(recomputedQ1, q1Metrics) || ...
        ~strcmp(recomputedInputSha, textField(q1, 'input_sha256', 64))
    error('rebuiltMexQualification:MetricRecomputationMismatch', ...
        'Stored Q1 evidence differs from the canonical comparator rerun.');
end
...
if testOnly
    parentReceipt = testContext.parent_receipt;
else
    parentReceipt = validate_q2_parent_fields(q2LockPath, ...
        parentLockPath, parentRoot);
end
if ~isstruct(parentReceipt) || ~isfield(parentReceipt, 'status') || ...
        ~strcmp(parentReceipt.status, 'ready')
    error('rebuiltMexQualification:Q2Blocked', ...
        'Canonical parent evidence is not ready for Q2 recomputation.');
end
candidate = q2_candidate_fields_from_state(parentReceipt, q2State);
recomputedQ2 = compare_q2_cycle1_fields(parentReceipt, candidate, q2Masks);
if ~semanticEqual(recomputedQ2, q2Metrics)
    error('rebuiltMexQualification:MetricRecomputationMismatch', ...
        'Stored Q2 metrics differ from locked parent/state recomputation.');
end
validateEvidenceIdentities(evidence, runtime, runtimeLockSha, q2LockSha, ...
    parentLockSha, q1, q2);
...
receipt = orderfields(struct( ...
    ...
    'qualification_evidence_lock_sha256', expectedEvidenceLockSha256, ...
...
if testOnly
    receipt.authorization_scope = 'test_only_non_authorizing';
    receipt = orderfields(receipt);
end
if ~isempty(outputPath), publishJsonNoClobber(outputPath, receipt); end
end

function evidence = validateEvidenceLock(path, expectedSha, qualificationRoot)
if ~isfile(path) || ~strcmp(fileSha256(path), expectedSha)
    error('rebuiltMexQualification:EvidenceLockMismatch', ...
        'Qualification evidence lock path or pinned digest does not match.');
end
evidence = readJson(path, 'rebuiltMexQualification:EvidenceLockMismatch');
requireExactFields(evidence, {'schema_version', ...
    'sealed_after_real_task6_qualification','identities','artifacts'});
requireSchema(evidence, 'rebuilt_mex_qualification_evidence_lock_v1');
if ~trueLogical(evidence, 'sealed_after_real_task6_qualification')
    error('rebuiltMexQualification:EvidenceLockUnsealed', ...
        'Qualification evidence lock is not a real Task6 seal.');
end
names = {'q1_metrics','q1_result','q1_receipt','q2_state', ...
    'q2_terminal','q2_metrics','q2_parent_masks','q2_result'};
...
if ~isstruct(evidence.artifacts) || ~isscalar(evidence.artifacts) || ...
        ~isequal(sort(fieldnames(evidence.artifacts)), sort(names(:)))
    artifactMismatch('Evidence-lock artifact inventory is not exact.');
end
root = normalizePath(qualificationRoot);
for index = 1:numel(names)
    record = evidence.artifacts.(names{index});
    requireExactFields(record, {'relative_path','sha256','bytes'});
    if ~strcmp(char(record.relative_path), relative{index})
        artifactMismatch('Evidence-lock artifact path differs from canonical path.');
    end
    artifactPath = fullfile(qualificationRoot, ...
        strrep(relative{index}, '/', filesep));
    if ~startsWith(normalizePath(artifactPath), [root, '/']) || ...
            ~isfile(artifactPath)
        artifactMismatch('Evidence-lock artifact is absent or outside qualification root.');
    end
    details = dir(artifactPath);
    if ~isnumeric(record.bytes) || ~isscalar(record.bytes) || ...
            record.bytes ~= details.bytes || ...
            ~strcmp(textField(record, 'sha256', 64), fileSha256(artifactPath))
        artifactMismatch('Evidence-lock artifact hash or size changed.');
    end
end
end

function validateEvidenceIdentities(evidence, runtime, runtimeLockSha, ...
        q2LockSha, parentLockSha, q1, q2)
names = {'runtime_lock_sha256','runtime_initial_sha256','source_commit', ...
    ...
    'parent_vtk_mesh_sha256','q1_input_sha256'};
requireExactFields(evidence.identities, names);
identity = evidence.identities;
pairs = { ...
    textField(identity,'runtime_lock_sha256',64), runtimeLockSha; ...
    textField(identity,'runtime_initial_sha256',64), runtime.initial_sha256; ...
    textField(identity,'source_commit',40), runtime.source_commit; ...
    textField(identity,'q2_input_lock_sha256',64), q2LockSha; ...
    textField(identity,'parent_lock_sha256',64), parentLockSha; ...
    textField(identity,'parent_cycle1_sha256',64), q2.parent_cycle1_sha256; ...
    textField(identity,'mask_set_sha256',64), q2.mask_set_sha256; ...
    textField(identity,'mesh_ordering_sha256',64), q2.mesh_ordering_sha256; ...
    textField(identity,'replay_source_mesh_sha256',64), ...
        q2.replay_source_mesh_sha256; ...
    textField(identity,'parent_vtk_mesh_sha256',64), q2.parent_vtk_mesh_sha256; ...
    textField(identity,'q1_input_sha256',64), q1.input_sha256};
if ~all(cellfun(@(left,right) strcmp(char(left),char(right)), ...
        pairs(:,1), pairs(:,2)))
    identityMismatch('Evidence-lock identity differs from canonical evidence.');
end
end

function requireTestOnlyCaller()
stack = dbstack('-completenames');
expected = normalizePath(fullfile(fileparts(mfilename('fullpath')), ...
    'validate_qualification_receipts_test_only.m'));
if numel(stack) < 3 || ~strcmp(normalizePath(stack(3).file), expected)
    error('rebuiltMexQualification:TestOnlyEntryRejected', ...
        'Internal synthetic qualification context is not a production API.');
end
end
