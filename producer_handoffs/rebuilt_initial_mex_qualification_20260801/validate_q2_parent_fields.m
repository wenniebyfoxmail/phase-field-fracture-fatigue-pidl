...
meshBridge = validateParentIdentity(q2Lock.parent, parentLock, ...
    q2Lock.mesh_bridge, parentRoot);
...
receipt = orderfields(struct( ...
    'schema_version', 'rebuilt_initial_mex_q2_parent_receipt_v2', ...
    'status', status, ...
    'blockers', {blockers}, ...
    ...
    char(meshBridge.replay_source_mesh_sha256), ...
    'replay_source_mesh_sha256_semantics', ...
    char(meshBridge.replay_source_mesh_sha256_semantics), ...
    'parent_vtk_mesh_sha256', char(meshBridge.parent_vtk_mesh_sha256), ...
    'parent_vtk_mesh_sha256_semantics', ...
    char(meshBridge.parent_vtk_mesh_sha256_semantics), ...
    'mesh_bridge_contract', meshBridge, ...
...
required = {'schema_version', 'parent_lock_sha256', 'parent', ...
    ...
    'prohibited_substitutes', 'mesh_bridge'};
...
validateMeshBridgeLock(lock.mesh_bridge, lock.parent.num_elem);
end

function meshBridge = validateParentIdentity(expected, parent, meshBridge, parentRoot)
...
if ~strcmp(parent.mesh.task4_parent_mesh_sha256, ...
        meshBridge.parent_vtk_mesh_sha256) || ...
        meshBridge.num_elem ~= expected.num_elem
    error('rebuiltMexQ2:ParentIdentityMismatch', ...
        'The locked parent VTK bridge differs from PARENT_LOCK.json.');
end
parsed = read_q2_parent_vtk_mesh(parentRoot, ...
    meshBridge.parent_vtk_relative_path);
if ~strcmp(parsed.file_sha256, meshBridge.parent_vtk_sha256) || ...
        ~strcmp(parsed.mesh_sha256, meshBridge.parent_vtk_mesh_sha256) || ...
        parsed.num_node ~= meshBridge.num_node || ...
        parsed.num_elem ~= meshBridge.num_elem
    error('rebuiltMexQ2:ParentVtkIdentityMismatch', ...
        'Independently parsed parent VTK evidence differs from Q2_INPUT_LOCK.');
end
...
function validateMeshBridgeLock(bridge, numElem)
...
semantics = ...
    'sha256_matlab_column_major_float64_coords_then_int64_connectivity_v1';
if ~isstruct(bridge) || ~all(isfield(bridge, required)) || ...
        ~strcmp(bridge.semantics_id, ...
        'q2_source_to_parent_vtk_mesh_bridge_v1') || ...
        ~isSha256(bridge.replay_source_mesh_sha256) || ...
        ~isSha256(bridge.parent_vtk_sha256) || ...
        ~isSha256(bridge.parent_vtk_mesh_sha256) || ...
        ~strcmp(bridge.replay_source_mesh_sha256_semantics, semantics) || ...
        ~strcmp(bridge.parent_vtk_mesh_sha256_semantics, semantics) || ...
        ~strcmp(bridge.connectivity_requirement, ...
        'exact_q4_row_and_local_node_order') || ...
        bridge.num_elem ~= numElem || bridge.num_node < 1 || ...
        bridge.coordinate_max_abs_tolerance ~= 1e-14 || ...
        bridge.reference_coordinate_max_abs_error < 0 || ...
        bridge.reference_coordinate_rms_error < 0
    error('rebuiltMexQ2:InvalidInputLock', ...
        'The Q2 source-to-parent VTK mesh bridge is not sealed.');
end
end
