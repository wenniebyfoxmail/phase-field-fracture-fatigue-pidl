classdef rebuiltMexQ2ParentTest < matlab.unittest.TestCase
    methods (Test)
        ...
        function testRejectsChangedPredeclaredBridgeTolerance(testCase)
            fixture = makeParentFixture(testCase, 'complete');
            q2Lock = jsondecode(fileread(fixture.q2_lock));
            q2Lock.mesh_bridge.coordinate_max_abs_tolerance = 1e-12;
            writeJson(fixture.q2_lock, q2Lock);
...
meshCoords = [0 0; 1 0; 2 0; 3 0; 0 1; 1 1; 2 1; 3 1];
meshConnectivity = [1 2 6 5; 2 3 7 6; 3 4 8 7];
sourceFiles = makeEightFiles(parentRoot, cyclePath, ...
    meshCoords, meshConnectivity);
meshHash = canonicalMeshHash(meshCoords, meshConnectivity);
...
q2Lock = struct( ...
    ...
    'sha256_matlab_column_major_float64_coords_then_int64_connectivity_v1', ...
    'parent_vtk_relative_path', 'peak_load_c1.vtk', ...
    'parent_vtk_sha256', sourceFiles.peak_load_c1_vtk.sha256, ...
    'parent_vtk_mesh_sha256', meshHash, ...
    'parent_vtk_mesh_sha256_semantics', ...
    'sha256_matlab_column_major_float64_coords_then_int64_connectivity_v1', ...
    'connectivity_requirement', 'exact_q4_row_and_local_node_order', ...
    'num_node', size(meshCoords, 1), 'num_elem', size(meshConnectivity, 1), ...
    'coordinate_max_abs_tolerance', 1e-14, ...
    'reference_coordinate_max_abs_error', 0, ...
    'reference_coordinate_rms_error', 0), ...
...
function sourceFiles = makeEightFiles(parentRoot, cyclePath, coords, connectivity)
...
for index = 1:numel(names)
    ...
    if strcmp(name, 'peak_load_c1.vtk')
        writeLegacyQ4Vtk(path, coords, connectivity);
    elseif ~strcmp(path, cyclePath)
...
function writeLegacyQ4Vtk(path, coords, connectivity)
fileId = fopen(path, 'w');
