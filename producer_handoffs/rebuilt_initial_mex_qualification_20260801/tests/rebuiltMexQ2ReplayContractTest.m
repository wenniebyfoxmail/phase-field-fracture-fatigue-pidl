...
verifyFalse(testCase, isfile(fullfile(config.output_root, ...
    'Q2_PARENT_MASKS.mat')));
verifyEqual(testCase, receipt.replay_mesh_sha256, ...
...
verifyEqual(testCase, result.mesh_ordering_sha256, ...
    context.contract.parent_mesh_sha256);
...
replay = struct('status', 'q2_cycle1_diagnostic_complete', ...
    ...
    'mesh_ordering_sha256', parent.replay_mesh_sha256);
...
verifyEqual(testCase, receipt.mesh_ordering_sha256, parent.replay_mesh_sha256);
...
contract = struct( ...
    ...
    'parent_mesh_sha256', canonicalMeshHash(mesh.node, mesh.elem));
context = struct('output_root', outputRoot, 'contract', contract, ...
    ...
    'identity', replayIdentity(contract.parent_mesh_sha256));
end
...
input = struct( ...
    ...
    'parent_mesh_sha256', canonicalMeshHash(nodeCoords, connectivity));
end
...
parent = struct('schema_version', ...
    'rebuilt_initial_mex_q2_parent_receipt_v1', 'status', 'ready', ...
    ...
    'replay_mesh_sha256', repmat('5', 1, 64), ...
    'num_elem', n, 'fields', fields);
end

function identity = replayIdentity(meshHash)
